...
/**
 * Implementación de ejemplo de BaseBungee con sistemas de emotes, economía y métricas
 * Esta clase extiende BaseBungee e implementa funcionalidades específicas del plugin
 */
public class ExampleBaseBungeeImpl extends BaseBungee {
    ...
    public ExampleBaseBungeeImpl(BasePlugin plugin) {
        super(plugin);
    ...
    public void enable() {
        ...
        loadConfigurations();
        initializeMetrics();
        getPlugin().getLogger().info("ExampleBaseBungeeImpl habilitado con sistemas de emotes, economía y métricas");
    }
    ...
    public void disable() {
        saveAllData();
        super.disable();
        getPlugin().getLogger().info("ExampleBaseBungeeImpl deshabilitado - datos guardados");
    }
    ...
    public void reload() {
        super.reload();
        loadConfigurations();
        getPlugin().getLogger().info("ExampleBaseBungeeImpl recargado");
    }
     ...
     */
    public boolean hasEmoteAccess(ProxiedPlayer player, String emoteName) {
     ...
     */
    public void useEmote(ProxiedPlayer player, String emoteName) {
        ...
        if (emoteSymbol != null && hasEmoteAccess(player, emoteName)) {
            ...
            for (ProxiedPlayer onlinePlayer : getPlugin().getProxy().getPlayers()) {
                Audience audience = audienciePlayer(onlinePlayer);
     ...
     */
    public double getBalance(ProxiedPlayer player) {
     ...
     */
    public void setBalance(ProxiedPlayer player, double amount) {
     ...
     */
    public void addBalance(ProxiedPlayer player, double amount) {
     ...
     */
    public boolean removeBalance(ProxiedPlayer player, double amount) {
        double currentBalance = getBalance(player);
     ...
     */
    public boolean transferMoney(ProxiedPlayer from, ProxiedPlayer to, double amount) {
        if (removeBalance(from, amount)) {
            addBalance(to, amount);
            
            Audience fromAudience = audienciePlayer(from);
            Audience toAudience = audienciePlayer(to);
    ...
    public Map<String, Object> getAllMetrics() {
        ...
        allMetrics.put("online_players", getPlugin().getProxy().getOnlineCount());
        return allMetrics;
    }
     ...
     */
    public void showServerStats(ProxiedPlayer player) {
        Audience audience = audienciePlayer(player);
        
        audience.sendMessage(Component.text("=== ESTADÍSTICAS DEL SERVIDOR ===").color(NamedTextColor.GOLD));
        audience.sendMessage(Component.text("Jugadores conectados: " + getPlugin().getProxy().getOnlineCount()).color(NamedTextColor.YELLOW));
